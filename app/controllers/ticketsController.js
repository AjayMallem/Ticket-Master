...
module.exports.list = (req,res) => {
    Ticket.find({userId: req.user._id}).populate('customer').populate('department').populate('employees')
        .then(tickets=>{
            res.json(tickets)
        })
        .catch(err=>{
...
module.exports.show = (req,res) => {
    ...
    Ticket.findOne({userId: req.user._id, _id: id}).populate('customer').populate('department').populate('employees')
        .then(ticket=>{
            if (ticket){
                res.json(ticket)
            } else {
